...
class DiscordScreenshot {
    constructor(text) {
        const proposed_width = 75 + (stringPixelWidth(text)) / 5;
        if (proposed_width < 300)
        {
            this.WIDTH = 300
        }
        else
        {
            this.WIDTH = proposed_width
        }
        this.HEIGHT = constants.HeightMeasuredWith;
    ...
    async setPfp(image) {
        this.pfp = await preparePfp(image, this.HEIGHT / 1.2);
    ...
    async setUsername(text, color, font = fonts.then(x => x.semibold)) {
        ...
        this.username.recolor(color[0], color[1], color[2]);
    ...
    async setTimestamp(textOrDate, font = fonts.then(x => x.medium)) {
        ...
        this.timestampLocation = new Location(
            ...
            this.usernameLocation.y +
                ...
                (0 * this.HEIGHT) / constants.HeightMeasuredWith
        );
        return this;
    }
